...
# Function to create 999 "help" files on the desktop
def create_help_files():
    # Get the desktop path based on the user's OS
    desktop_path = Path(os.path.join(os.path.expanduser('~'), 'Desktop'))

    # Create 999 files named "help.txt"
    for i in range(1, 1000):
        file_path = desktop_path / f"help_{i}.txt"
        with open(file_path, 'w') as file:
            file.write(f"This is help file number {i}")
...
def main():
    ...
    display_image(person_image)
    second_text = "Oh. The virus is trying to add 999 items to your desktop. Watch out!"
